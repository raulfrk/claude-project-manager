...
class TestDiscoverAndRegister:
    ...
    def test_multiple_plugins(self, tmp_path: Path):
        ...
        stats = discover_and_register(registry, root=tmp_path)
        assert stats == {
            "perms": {"registered": 1, "updated": 0},
            "proj": {"registered": 1, "updated": 0},
        }
    ...
    def test_registers_verification_hook(self, tmp_path: Path):
        """Hooks with verification: true are registered with
        verification=True and blocking forced."""
        _setup_plugin(
            tmp_path,
            "todoist",
            hooks=[
...
class TestHookContentDiffers:
    ...
    def test_param_mapping_differs(self):
        hook = Hook(id="1", trigger_tool="a", target_tool="b", server="s", param_mapping={"x": "1"})
