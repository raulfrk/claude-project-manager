...
**2.** Call `mcp__plugin_worktree_worktree__wt_add_repo` with `path=<path>`, `label=<label>`, `default_branch=<default_branch or "main">`.
   - If the path is not a git repository: output "Path '<path>' is not a git repository. Verify the path contains a .git directory." and stop.
   - If the label is already registered: output "Label '<label>' already registered. Run `/worktree:list` to see existing repos." and stop.
