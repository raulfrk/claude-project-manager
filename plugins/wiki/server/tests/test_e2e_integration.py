...
class TestWikiProjIntegration:
    @pytest.mark.anyio()
    async def test_scope_detect_no_session(self, integrated_home: Path) -> None:
        """proj.yaml exists but no session file → global."""
        result = json.loads(await wiki_scope.wiki_scope_detect())
        assert result == {"scope": "global", "proj_present": True}

    @pytest.mark.anyio()
    async def test_scope_detect_with_active_project(self, integrated_home: Path) -> None:
    ...
    @pytest.mark.anyio()
    async def test_scope_detect_session_without_proj(
    ...
    @pytest.mark.anyio()
    async def test_full_round_trip_active_project_flows_through(
        self, integrated_home: Path
    ) -> None:
        ...
        result_after_clear = json.loads(await wiki_scope.wiki_scope_detect())
