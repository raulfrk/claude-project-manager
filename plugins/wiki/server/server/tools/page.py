...
async def wiki_page_get(slug: str, category: str | None) -> str:
    ...
    def _do_get() -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"malformed frontmatter: {e}", "path": str(target)}
...
async def wiki_page_list(
    ...
    def _do_list() -> dict[str, Any]:
        ...
        return {"pages": results, "truncated": truncated}

    return json.dumps(await anyio.to_thread.run_sync(_do_list))
