...
async def test_scope_sessions(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="authentication", scope="sessions"))
...
async def test_scope_notes(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="authentication", scope="notes"))
    assert result["total_matches"] >= 1
    sources = [s["source"] for s in result["snippets"]]
...
async def test_scope_requirements(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="OAuth2", scope="requirements"))
    assert result["total_matches"] >= 1
    sources = [s["source"] for s in result["snippets"]]
...
async def test_scope_research(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="authlib", scope="research"))
    assert result["total_matches"] >= 1
    sources = [s["source"] for s in result["snippets"]]
...
async def test_scope_decisions(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="authlib", scope="decisions"))
    assert result["total_matches"] >= 1
    sources = [s["source"] for s in result["snippets"]]
...
async def test_scope_all_searches_all_stores(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="authentication", scope="all"))
    sources = {s["source"] for s in result["snippets"]}
...
async def test_regex_matching(knowledge_app, project: str) -> None:
    ...
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="OAuth\\d?", scope="all"))
    assert result["total_matches"] >= 1


@pytest.mark.asyncio
async def test_context_lines_included(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="login bug", scope="sessions"))
    assert result["total_matches"] >= 1
...
async def test_max_snippets_limit(cfg: ProjConfig, knowledge_app, tmp_path: Path) -> None:
    ...
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="keyword", scope="notes"))
...
async def test_empty_results(knowledge_app, project: str) -> None:
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="zzz_nonexistent_term", scope="all"))
...
async def test_missing_sessions_dir(cfg: ProjConfig, knowledge_app, tmp_path: Path) -> None:
    ...
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="anything", scope="sessions"))
    assert result["total_matches"] == 0
    assert result["snippets"] == []
...
async def test_missing_todos_dir(cfg: ProjConfig, knowledge_app, tmp_path: Path) -> None:
    ...
    for scope in ("requirements", "research"):
        result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="anything", scope=scope))
        assert result["total_matches"] == 0
        assert result["snippets"] == []
...
async def test_missing_decisions_file(cfg: ProjConfig, knowledge_app, tmp_path: Path) -> None:
    ...
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="anything", scope="decisions"))
    assert result["total_matches"] == 0
    assert result["snippets"] == []
...
async def test_snippets_sorted_by_density(cfg: ProjConfig, knowledge_app, tmp_path: Path) -> None:
    ...
    result = json.loads(await _call(knowledge_app, "proj_search_knowledge", query="word", scope="notes"))
    assert result["total_matches"] >= 2
