...
- If no `trello_card_id` is set, stop with: "No Trello card linked. Run `/proj:trello-setup` to link one."
...
- **No Trello card linked**: displays "No Trello card linked. Run `/proj:trello-setup` to link one." and stops.
- **Trello MCP unavailable**: displays error from tool call and stops.

## Output

JSON object `{"checklists": [...]}` containing the card's checklists with their items. Returned for use by downstream sub-skills (trello-diff).
