...
**Step 1**: Call `mcp__hooks__hooks_list_tool` to look up the hook by ID. Find the hook entry whose `id` matches. If not found, output "Hook `<hook_id>` not found." and stop.

**Step 2**: Extract the `trigger_tool` from the matched hook.

**Step 3**: Call `mcp__hooks__hooks_fire_tool` with:
- `trigger_tool` = the hook's trigger_tool
- `source_result` = the provided source_result_json or `{}`

**Step 4**: Parse the JSON response and display results:
...
Suggest: "Use `/proj:hooks-debug` to inspect failures." if errors occurred.
