...
allowed-tools: mcp__plugin_hooks_hooks__hooks_fire_tool, mcp__plugin_hooks_hooks__hooks_list_tool
...
**1.** Call `mcp__plugin_hooks_hooks__hooks_list_tool` to look up the hook by ID. Find the hook entry whose `id` matches. If not found, stop with: "Hook `<hook_id>` not found. Run `/proj:hooks-list` to see available hooks."

**2.** Extract the `trigger_tool` from the matched hook.

**3.** Call `mcp__plugin_hooks_hooks__hooks_fire_tool` with:
