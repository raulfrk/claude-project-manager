...
allowed-tools: mcp__plugin_sandbox_sandbox__sandbox_batch_setup, mcp__plugin_sandbox_sandbox__sandbox_set_deny, mcp__plugin_sandbox_sandbox__sandbox_reconcile, mcp__plugin_proj_proj__config_update, mcp__plugin_proj_proj__proj_list, mcp__plugin_proj_proj__proj_get, mcp__plugin_sandbox_sandbox__sandbox_list, mcp__plugin_sandbox_sandbox__sandbox_list, mcp__plugin_sandbox_sandbox__sandbox_reconcile, mcp__plugin_sandbox_sandbox__sandbox_list, mcp__plugin_sandbox_sandbox__sandbox_list, mcp__plugin_proj_proj__proj_get_active, mcp__plugin_proj_proj__config_load, mcp__plugin_worktree_worktree__wt_config_get, Bash
...
**1b.** Call `sandbox_list` with `scope="user"` and `target="auto"` to get all current rules.
...
**1c.** Call `sandbox_list` to check current sandbox state.
...
Call `sandbox_list`. Record the returned timestamp.
...
- Derive expected MCP servers from config:
  ...
  - If `sandbox_integration: true`: `plugin_sandbox_sandbox`
...
Call `sandbox_reconcile`. This strips all Read/Edit/Bash rules from BOTH settings files, keeping only MCP and WebFetch rules.

Display before/after counts.

On failure: display "Migration failed at step 4. Restore with: `sandbox_list('<timestamp>')`" and **stop**.

**5.** Replace sandbox paths

Call `sandbox_batch_setup` with:
- `paths` = the computed `sandbox_paths` from step 3
- `preserve_extra` = `true` (merge with user-added paths detected in step 1c)

On failure: display "Migration failed at step 5. Restore with: `sandbox_list('<timestamp>')`" and **stop**.

**6.** Reconcile MCP wildcards

Call `sandbox_reconcile` with:
...
This removes stale MCP wildcards from BOTH files and adds missing expected wildcards to settings.json only. All non-stale MCP rules (user-managed servers) are preserved.

On failure: display "Migration failed at step 6. Restore with: `sandbox_list('<timestamp>')`" and **stop**.

**7.** Install deny rules

**7a.** Call `sandbox_set_deny` with the DEFAULT_DENY_RULES. This writes deny rules to settings.json.
...
(1) Clear deny rules from settings.json (recommended — consolidate in settings.json)
(2) Keep deny rules in settings.json (both files will have deny rules)
(3) Skip deny rule installation entirely
(4) Abort migration
```

If user picks 1: clear `permissions.deny` from `settings.json` via `sandbox_set_deny` targeting that file.
...
If user picks 4: display "Migration aborted at step 7. Restore with: `sandbox_list('<timestamp>')`" and **stop**.

On failure: display "Migration failed at step 7. Restore with: `sandbox_list('<timestamp>')`" and **stop**.
...
On failure: display "Migration failed at step 8. Restore with: `sandbox_list('<timestamp>')`" and **stop**.

**9.** Verify

Call `sandbox_list` with `scope="user"` and `target="auto"` to get the final state.
Call `sandbox_list` to confirm sandbox is enabled.
...
Call `sandbox_list` with the provided timestamp.
...
- **Backup failure**: displays error from `sandbox_list` and stops.
- **Any step failure**: displays which step failed and rollback instructions with the backup timestamp.
- **Invalid restore timestamp**: displays error from `sandbox_list` and stops.
