...
**1.** If $ARGUMENTS is empty:
   ...
   - If the list is empty, stop: "No tracked projects found. Run `/proj:init` to add one."
   - Present a numbered list and ask the user to pick one
   - Call `mcp__proj__proj_load_session` with the selected name
   - If the tool returns not-found, display the error and suggest: "Check the project name or run `/proj:init` to add it."
