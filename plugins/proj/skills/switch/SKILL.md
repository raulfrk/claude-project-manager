...
**1.** Call `mcp__proj__proj_list` to get all tracked projects.
   - If the list is empty: reply "No tracked projects. Run `/proj:init` to add one." and stop.
...
**2.** If $ARGUMENTS is empty, present the list and ask the user which project to switch to.
   ...
   - If no match: reply "Project '<name>' not found. Run `/proj:list` to see available projects." and stop.
