...
1. Call `mcp__proj__proj_get_todo_context` with the todo ID to get the todo, requirements, and research in one call. If the todo has a non-empty `notes` field, treat it as additional context (e.g. description pulled from Todoist) — it may contain design constraints or prior decisions that should inform your research.
...
4. Write a structured `research.md`:
   ```markdown
   # Research: <todo title>
   ...
   - <link or file path>
   ```
5. Call `mcp__proj__content_set_research` with the content.
6. Call `mcp__proj__todo_set_content_flag` with `has_research=True`.
