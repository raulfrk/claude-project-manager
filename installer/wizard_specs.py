...
PROJ_YAML_PROMPTS: list[PromptSpec] = [
    ...
    PromptSpec(
        ...
        dotted_key="smart_gate",
        type="bool",
        group="Smart gate",
        tier="advanced",
        default_factory=_d("smart_gate"),
    ),
    ...
    PromptSpec(
        ...
        group="Trello",
        tier="basic",
        default_factory=_d("sync.trello.default_board_id"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    # ---------- Basic tier (proj.yaml) — sync.trello label names ----------
    # Naming footgun: label name is SINGULAR (`proj-task`), list name is PLURAL
    # (`proj-tasks`, in list_mappings.tasks). One-character difference.
    PromptSpec(
        label="Trello label — project cards (singular: 'proj-task', not plural 'proj-tasks')",
        dotted_key="sync.trello.proj_label_name",
        type="str",
        group="Trello labels",
        tier="basic",
        default_factory=_d("sync.trello.proj_label_name"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    PromptSpec(
        label="Trello label — todo cards (singular: 'proj-task', not plural 'proj-tasks')",
        dotted_key="sync.trello.proj_task_label_name",
        type="str",
        group="Trello labels",
        tier="basic",
        default_factory=_d("sync.trello.proj_task_label_name"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    ...
    PromptSpec(
        ...
        tier="advanced",
        default_factory=_d("sync.trello.list_mappings.created"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    PromptSpec(
        label="Trello list — completed todos",
    ...
    PromptSpec(
        label="Trello list — project tracking cards",
        dotted_key="sync.trello.list_mappings.projects",
        type="str",
        group="Trello list mappings",
        tier="advanced",
        default_factory=_d("sync.trello.list_mappings.projects"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    PromptSpec(
        label="Trello list — active todo cards",
        dotted_key="sync.trello.list_mappings.tasks",
        type="str",
        group="Trello list mappings",
        tier="advanced",
        default_factory=_d("sync.trello.list_mappings.tasks"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    PromptSpec(
        label="Trello list — active projects",
        dotted_key="sync.trello.list_mappings.active",
        type="str",
        group="Trello list mappings",
        tier="advanced",
        default_factory=_d("sync.trello.list_mappings.active"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    PromptSpec(
        label="Trello list — paused/blocked projects",
        dotted_key="sync.trello.list_mappings.pending",
        type="str",
        group="Trello list mappings",
        tier="advanced",
        default_factory=_d("sync.trello.list_mappings.pending"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
    PromptSpec(
        label="Trello list — archived projects",
        dotted_key="sync.trello.list_mappings.archived",
        type="str",
        group="Trello list mappings",
        tier="advanced",
        default_factory=_d("sync.trello.list_mappings.archived"),
        condition=lambda ex: bool(get_nested(ex, "sync.trello.enabled", False)),
    ),
]
