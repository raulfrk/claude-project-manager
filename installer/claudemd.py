...
- **Team agent shutdown** — After all agents complete: (1) send `{{"type": "shutdown_request", "reason": "done"}}` via `SendMessage` to each agent individually (broadcast does not support structured messages), (2) wait ~30s for terminations, (3) call `TeamDelete`. If `TeamDelete` fails, restart Claude. ~90% of agents terminate on first request; `claude-code-guide` subagent types tend to ignore it. Always persist critical state (commits, MCP calls) BEFORE shutdown — never rely on clean shutdown for correctness.
